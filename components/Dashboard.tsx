...
export const Dashboard: React.FC<DashboardProps> = ({ teacher, onLogout }) => {
  ...
  const handleShareNote = (note: ClassNote) => {
    // 1. Find Subject from Schedule using Session ID
    let subject = '';
      ...
      <div className="space-y-4">
        {sortedNotes.map(note => (
           <div key={note.id} className="bg-white rounded-lg border-l-4 border-yellow-400 shadow-sm p-4 relative min-h-[100px] overflow-hidden">
              ...
              <button 
                 type="button"
                 onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleShareNote(note); }} 
                 className="absolute top-2 right-12 p-2 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded-full transition-colors z-10"
